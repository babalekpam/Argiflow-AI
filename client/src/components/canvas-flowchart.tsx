...
function EdgeLabel({ x, y, text, type }: { x: number; y: number; text: string; type: string }) {
  ...
  return (
    <g>
      <rect x={x - 14} y={y - 8} width="28" height="16" rx="4" fill={bgColor} opacity="0.9" />
      <text
        x={x}
        y={y + 4}
        textAnchor="middle"
        ...
        >
          ...
          {displayNodes.map((node, i) => {
            ...
            return (
              ...
              >
                {node.type === "decision" ? (
                  <DiamondShape node={node} isActive={isActive} onClick={() => handleNodeClick(node.id)} />
                ) : (
                  <RectNode node={node} isActive={isActive} onClick={() => handleNodeClick(node.id)} />
                )}
              </g>
            );
          })}
