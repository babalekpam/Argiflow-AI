...
- **AI Agent Console**: Run 6 built-in agents (Lead Scout, Cold Email Writer, Reply Analyzer, Intent Monitor, Meeting Booker, Forum Prospector) with any configured LLM provider. Each agent has comprehensive universal system prompts with geographic intelligence (US, Africa, LatAm, Europe), industry-specific tone guides, structured output formats, and objection handling. API: `/api/agent-console/run` (body: agentId+prompt) or `/api/agent-console/:id/run`. DB: `agent_runs` table.
