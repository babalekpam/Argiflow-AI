...
- **AI Marketing Suite**: 33-skill AI marketing toolkit powered by Anthropic Claude. 8 tools: Cold Email Builder, Email Sequence Designer, Landing Page Auditor, AI Copywriter, Launch Planner, Pricing Advisor, Ad Creative Generator, Sequence Reviewer. Auto-detects intent and injects relevant skills. Backend: `server/marketing-suite-routes.ts`, skills: `server/marketing-skills/` (skills-data.ts, skills-loader.ts, product-context.md). Frontend: `client/src/pages/marketing-suite.tsx` at `/dashboard/marketing-suite`. API: `/api/marketing-suite/*` (auth required). Zod-validated inputs.
