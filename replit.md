...
- **Visitor Tracking & Analytics Engine**: Embeddable JavaScript snippet for detailed website visitor intelligence, tracking page views, sessions, clicks, form behavior, and custom events. Includes **Visitor Intelligence Engine** (`server/visitor-intelligence.ts`) that cross-references anonymous visitor IPs with email tracking data (open/click IPs) to identify visitors, matches them to existing leads, computes intent scores based on pages viewed and CTAs clicked, and feeds this intelligence into Abel's decision-making. **Email-to-Website Identity Bridge**: When email recipients click links, the click redirect appends `_afref=TOKEN` to the destination URL; the tracker snippet (`argiflow-tracker.js`) detects this and calls `/api/tracker/email/resolve` to link the visitor identity (verified via IP match against click event). Sessions store `from_email_token` for deterministic attribution. Abel sees the full email→website journey: which emails they opened, what links they clicked, and every page they browsed after arriving. API: `/api/aria/visitor-intelligence`.
