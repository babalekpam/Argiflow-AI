...
- **AI Business Manager**: An autonomous AI agent that runs the business on behalf of the tenant. Configurable with business context, goals, personality (professional/friendly/aggressive/consultative), autonomy level (conservative/moderate/aggressive), and 7 capability toggles (lead gen, email outreach, follow-ups, pipeline, marketing, analytics, inbox). Runs every 15 minutes when enabled — analyzes business state, makes decisions, executes actions, and generates daily reports. Backend: `server/business-manager.ts` + `server/business-manager-routes.ts`. Schema: `shared/business-manager-schema.ts` (tables: `business_manager`, `manager_decisions`, `manager_daily_reports`). API: `/api/business-manager/*`. Frontend: `/dashboard/business-manager`.
