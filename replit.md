...
- **White-Label Sending Domains**: Clients connect their own domain for branded email sending. DNS verification (SPF, DKIM, Return Path) with copy-paste records. Backend: `server/domain-engine.ts`, `server/domain-routes.ts`. Frontend: `client/src/pages/domain-setup.tsx` at `/dashboard/domain`. API: `/api/domains/*`. DB: `client_domains` table.
